...
function Band({
  ...
  // useTexture must be called unconditionally; reuse the (already-loaded)
  // lanyard band texture as a harmless fallback when a face image isn't
  // supplied, then skip compositing it below.
  const frontTex = useTexture(frontImage || LANYARD_PNG_URL);
  const backTex = useTexture(backImage || LANYARD_PNG_URL);
  ...
  const cardMap = useMemo(() => {
    ...
    const baseImg = baseMap.image as LoadedImage;
    ...
    const drawFitted = (img: LoadedImage, rect: typeof FRONT_UV_RECT) => {
      const rx = rect.x * W;
      const ry = rect.y * H;
      const rw = rect.w * W;
      const rh = rect.h * H;
    ...
    if (frontImage && frontTex.image) drawFitted(frontTex.image as LoadedImage, FRONT_UV_RECT);
    if (backImage && backTex.image) drawFitted(backTex.image as LoadedImage, BACK_UV_RECT);
  ...
  useEffect(() => {
    // three.js Texture instances are mutable by design — this is the only way
    // to configure wrap mode, there is no immutable constructor option for it.
    ...
    <>
      ...
      <mesh ref={band}>
        ...
        <meshLineMaterial
          ...
          // Only sample the band texture when a custom lanyardImage is supplied.
          // Otherwise render a solid color so the strap follows `bandColor`
          // (the default texture is a black band, which a tint can't lighten).
